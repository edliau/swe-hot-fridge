<script>
  ...
  let zoomScale = 2; // Zoom magnification level
  ...
  function handleMouseMove(event) {
    if (!zoomActive || !imageContainer || !zoomedImage) return;
    ...
    // Update background position of zoomed image
  ...
  <main class="flex-1 p-4 max-w-6xl mx-auto">
    ...
    {:else if product}
      <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div>
          <div class="bg-white rounded-lg shadow-md p-4 mb-4 h-96 relative"
               bind:this={imageContainer}
               on:mousemove={handleMouseMove}
               on:mouseleave={exitZoom}
               on:click={toggleZoom}
               on:keydown={(e) => e.key === 'Enter' && toggleZoom()}
               tabindex="0"
               role="button"
               aria-label={zoomActive ? "Exit zoom" : "Zoom in"}
               class:cursor-zoom-in={!zoomActive}
               class:cursor-zoom-out={zoomActive}>
            {#if product.image}
              <img src={product.image} 
                   alt={product.name} 
                   class="max-h-full max-w-full object-contain mx-auto transition-opacity duration-300"
                   class:opacity-100={!zoomActive}
                   class:opacity-0={zoomActive}>
              ...
              {#if zoomActive}
                <div class="absolute inset-0 z-10 pointer-events-none"
                     ...
                     style="background-image: url('{product.image}'); 
                            ...
                            background-size: {zoomScale * 100}%;
                            background-position: {zoomPosition.x}% {zoomPosition.y}%;">
                </div>
              {/if}
            {:else}
